...
```{r}
gtf_ACTB_trancsript_number <- gtf_ACTB$transcript_id %>% 
  unique() %>% 
  setdiff(NA) %>% 
  length()

gtf_ACTB_trancsript_number

#or length(na.omit(unique(gtf_ACTB$transcript_id)))
#or   subset(gtf_ACTB, type == "transcript")
```
...
human_symbols <- human_symbols[1:6]
```

- Use the `AnnotationDbi::select()` method to fetch the Entrez and Ensembl identifiers for those gene symbols.

```{r}
AnnotationDbi::select(org.Hs.eg.db, human_symbols, columns = c("ENTREZID", "ENSEMBL"), keytype = "SYMBOL")
```
...
```{r}
mapIds(org.Hs.eg.db, human_symbols, column = "ENSEMBL", keytype = "SYMBOL", multiVals = "first")
```
...
```{r}
mart <- useMart(biomart = "ENSEMBL_MART_ENSEMBL", host = "https://www.ensembl.org")
mart
```
...
```{r}
getBM(c("hgnc_symbol", "chromosome_name", "start_position", "end_position", "strand"), filters = "hgnc_symbol", values = human_symbols, mart = mart)
...
```{r}
assay_counts <- read.table("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/counts.csv", row.names = 1, sep = ",", header = T)
