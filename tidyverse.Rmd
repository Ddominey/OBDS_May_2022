---
title: "Tidyverse"
author: "Delaney C C Dominey-Foy"
date: '2022-05-13'
...
```{r}
iris.tsv <- write_tsv(x = iris_raw, "iris.tsv")
class(iris_raw) 
dim(iris_raw)
typeof(iris_raw)
```
...
iris_raw %>% 
group_by(species) %>% 
  summarise(sepal_length = mean(sepal_length), sepal_length_median = median(sepal_length), sepal_length_minimum = min(sepal_length), sepal_length_maximum = max(sepal_length))    
```
...
```{r}
iris_raw %>% 
  mutate(sepal_area = sepal_length * sepal_width) %>% slice_head(n=5)
...
# Copy the code chunk above and extend with more pipes
iris_raw %>% 
      mutate(sepal_area = sepal_length * sepal_width) %>% select(species, sepal_area)

```

- Subset the result to display the top 5 (highest number) observations by `sepal_area`.

```{r}
# Copy the code chunk above and extend with more pipes
iris_raw %>% 
  mutate(sepal_area = sepal_length * sepal_width) %>% slice_max(sepal_area, n=5)
```
...
```{r}
ggplot(iris_for_ggplot, aes(x = sepal_area, color = species)) + geom_histogram() + facet_wrap(~species, ncol =2, nrow=2)
...
iris_long2 <- iris_raw %>% 
      mutate(ID = as.character(seq(1, nrow(iris)))) %>% 
    pivot_longer(cols = where(is.numeric), values_to = "measurement", names_to = "dimension")
iris_long2
```
...
```{r}
library(readxl)
excel_sheets("/t1-data/project/obds/ddominey/projects/Tidyverse/EH2011.xlsx")

exprs_assay <- read_excel(path = "/t1-data/project/obds/ddominey/projects/Tidyverse/EH2011.xlsx", sheet = "exprs") 
...
- Another way is: exprs_assay_long <- exprs_assay %>% pivot_longer(-gene, names_to = "sample", values_to = "exprs") exprs_assay_long
...
```{r}
ggplot(exprs_assay_long, aes(x = value, colour = gene, fill = gene)) + geom_histogram() + facet_wrap(~gene)
...
```{r}
excel_sheets("/t1-data/project/obds/ddominey/projects/Tidyverse/EH2011.xlsx")
sample_info <- read_excel(path = "/t1-data/project/obds/ddominey/projects/Tidyverse/EH2011.xlsx", sheet = "sample_info")
